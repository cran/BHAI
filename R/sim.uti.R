#' Performs simulation of the outcome tree of urinary tract infection (UTI) for DALY calculation
#' 
...
#' @keywords internal
#' @noRd
sim.uti = function(nhai, npatients, la, loi, discharges, mccabe_life_exp, mccabe_scores_distr, ncases_by_stratum, sample_distr="rbetamix", age_prior, p_age=NULL, pop.level, mccabe_prior, estimate_loi) {
  ...
  duration_symptomatic_infection = 0.007
  ...
  secondary_bacteraemia_urosepsis_trans = sample.runif(19*2, 0.0542, 0.2, pop.level, names(ncases_hai))
  weight_secondary_bacteraemia_urosepsis = sample.rpert(19*2, 0.579, 0.727, 0.655, pop.level, names(ncases_hai))
  duration_secondary_bacteraemia_urosepsis = sample.runif(19*2, 0.027, 0.036, pop.level, names(ncases_hai))
  secondary_bacteraemia_urosepsis = list()
  for(i in names(ncases_hai)) {
    secondary_bacteraemia_urosepsis[[i]] = ncases_hai[[i]]*secondary_bacteraemia_urosepsis_trans[[i]]* # Transition probability
      weight_secondary_bacteraemia_urosepsis[[i]]*duration_secondary_bacteraemia_urosepsis[[i]]
  }
  output$secondary_bacteraemia_urosepsis = secondary_bacteraemia_urosepsis # sum(unlist(secondary_bacteraemia_urosepsis))
  #
  prob_death= sample.runif(19*2, 0.09, 0.2, pop.level, names(ncases_hai))
  ndeath_hai = list()
  for(i in names(ncases_hai)) {
    ndeath_hai[[i]] = ncases_hai[[i]]*secondary_bacteraemia_urosepsis_trans[[i]]*prob_death[[i]]
  }
  ...
  for(i in names(ncases_hai)) {
    post_traumatic_disorder_trans = secondary_bacteraemia_urosepsis_trans[[i]]*prob_post_traumatic_disorder[[i]]
  ...
  for(i in names(ncases_hai)) {
    cognitive_impairment_trans = secondary_bacteraemia_urosepsis_trans[[i]]*prob_cognitive_impairment[[i]]
  ...
  for(i in names(ncases_hai)) {
    physical_impairment_trans = secondary_bacteraemia_urosepsis_trans[[i]]*matrix(rep(1,19*2), ncol=2)
  ...
  renal_failure = list()
  for(i in names(ncases_hai)) {
    renal_failure_trans = secondary_bacteraemia_urosepsis_trans[[i]]*prob_renal_failure[[i]]
